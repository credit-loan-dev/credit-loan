...
import com.sixliu.user.DataScope;
import com.sixliu.user.JsqlpaserUtils;
...
public class DataPermissioniInterceptor implements Interceptor {
	...
	private MappedStatement buildNewMappedStatement(MappedStatement mappedStatement, DataScope dataScope) {
		...
		builder.keyProperty(
				mappedStatement.getKeyProperties() != null ? join(mappedStatement.getKeyProperties(), ",") : null);
			...
			if (statement instanceof Select) {
				...
				if (selectStatement.getSelectBody() instanceof PlainSelect) {
					...
					throw new RuntimeException(
							"(UNION,INTERSECT,MINUS,EXCEPT,WITH)等函数或者方式都不支持自动添加条件,请关闭自动过滤条件自行添加过滤条件！");
				}
			}
			// 抛出异常
